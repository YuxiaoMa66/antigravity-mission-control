...
| --- | --- | --- |
| A — Lightweight Flash-first | Control cost with the smallest adequate team | Prefer Gemini Flash at role-appropriate intensity; let Codex own roles where another AGY pass adds little value; keep GPT last. |
...
| C — Quality-first Gemini Flash High | Keep B's quality objective while using one Gemini tier | Use the newest exact `gemini-.*flash-high` slug for every AGY role and follow-up; review separation comes from process and Codex verification. |
...
`--avoid-family` does not apply to C because C requires Gemini for every AGY call. Gain independence there through separate conversations, adversarial prompts, and Codex verification. C is the quality-first B shape with the provider route fixed to Gemini Flash High.
