...
<p align="center"><img src="assets/release-v0.6.0.svg" width="920" alt="Antigravity Mission Control v0.6.0 operational control board"></p>

> **v0.6.0:** Stable release with easier job operations: `agy-mc status` takes `--limit` and repeatable `--state`, and `agy-mc prune --older-than` deletes old finished jobs. `agy-mc wait --timeout` accepts `d` (days). A job with a malformed status no longer crashes `status`, `wait`, `cancel` or `prune`. Runtime routing behavior remains the v0.3.0 Flash-first design.
>
...
3. If AGY is ready, install the stable npm package with `npx antigravity-mission-control@latest install`; use `@0.6.0` when you need a pinned release. If AGY is missing, explain the official `--install-agy` option and ask for separate approval before using it.
...
These v0.6.0 illustrations show command excerpts and workflow summaries. Paths, IDs and quota values are examples, not live screenshots.
