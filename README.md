...
<p align="center"><img src="assets/release-v0.4.2rc2.svg" width="920" alt="Antigravity Mission Control v0.4.2rc2 operational control board"></p>

> **v0.4.2rc2 (pre-release, npm `0.4.2-rc.2`):** Second release candidate of the Python CLI module split. It fixes guard gaps found in review: a failed background worker could be reported as a success, broad directories such as `/Users` could be trusted as workspaces, and uninstall could remove an unmanaged Skill or stop halfway. Stable stays v0.4.1 until this is promoted. Try it with `npx antigravity-mission-control@next install`. Runtime routing behavior remains the v0.3.0 Flash-first design.
>
...
3. If AGY is ready, install the stable npm package with `npx antigravity-mission-control@latest install`; use `@0.4.1` when you need a pinned release, or `@next` to try the v0.4.2rc2 pre-release. If AGY is missing, explain the official `--install-agy` option and ask for separate approval before using it.
...
These v0.4.2rc2 illustrations show command excerpts and workflow summaries. Paths, IDs and quota values are examples, not live screenshots.
...
For the v0.4.2rc2 pre-release, use `@v0.4.2rc2` instead of `@v0.4.1`.
