...
from antigravity_mission_control import jobs, routing
...
class RoutingTests(unittest.TestCase):
    def test_a_prefers_role_specific_flash_intensity(self):
        self.assertEqual(routing.select_model("scout", MODELS, strategy="A"), "gemini-3.8-flash-low")
        self.assertEqual(routing.select_model("implementer", MODELS, strategy="A"), "gemini-3.8-flash-medium")

    def test_b_is_quality_first_but_flash_first_for_implementer(self):
        self.assertEqual(routing.select_model("implementer", MODELS, strategy="B"), "gemini-3.8-flash-high")
        self.assertEqual(
            routing.select_model("reviewer", MODELS, avoid_family="gemini", strategy="B"),
    ...
    def test_c_routes_every_role_to_latest_flash_high(self):
        ...
        for role in routing.ROLES:
            with self.subTest(role=role):
                self.assertEqual(routing.select_model(role, models, strategy="C"), "gemini-3.10-flash-high")
        ...
        for strategy, order in {
            ...
            for index in order:
                with self.subTest(strategy=strategy, remaining=len(candidates)):
                    self.assertEqual(routing.select_model("implementer", candidates, strategy=strategy), MODELS[index]["id"])
            ...
            for role in routing.ROLES:
                with self.subTest(strategy=strategy, role=role):
                    with self.assertRaisesRegex(RuntimeError, "No AGY models"):
                        routing.select_model(role, excluded, strategy=strategy)
                    self.assertEqual(routing.select_model(role, excluded + [MODELS[-1]], strategy=strategy), MODELS[-1]["id"])

    def test_c_rejects_missing_high_and_family_exclusion(self):
        with self.assertRaisesRegex(RuntimeError, "no Gemini Flash High"):
            routing.select_model("reviewer", [MODELS[0]], strategy="C")
        with self.assertRaisesRegex(RuntimeError, "does not support --avoid-family"):
            routing.select_model("reviewer", MODELS, avoid_family="gemini", strategy="C")
    ...
    def test_non_high_execution_requires_confirmation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(jobs, "workspace_status", return_value={"trusted": True}), patch.object(jobs, "available_models", return_value=MODELS):
                ...
                self.assertEqual(jobs.prepare_run(args)["model"], args.model)
