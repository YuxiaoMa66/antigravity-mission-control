...
class ReleaseIntegrityTests(unittest.TestCase):
    def test_versions_are_synchronized(self):
        ...
        self.assertEqual(common.VERSION, "0.4.2rc1")
        self.assertEqual(__version__, common.VERSION)
        self.assertIn('version = "0.4.2rc1"', pyproject)
        self.assertEqual(package["version"], "0.4.2-rc.1")
        self.assertIn("const VERSION = '0.4.2-rc.1'", npm_cli)
        self.assertIn("const PYTHON_VERSION = '0.4.2rc1'", npm_cli)
    ...
    def test_current_release_assets_and_install_refs(self):
        self.assertTrue((ROOT / "assets" / "release-v0.4.2rc1.svg").is_file())
        self.assertTrue((ROOT / "docs" / "visual-design-v0.4.2rc1.md").is_file())
        for document_name in (
            ...
            "docs/releases/v0.4.2rc1.md",
            "docs/releases/v0.4.2rc1.zh-CN.md",
        ):
            text = (ROOT / document_name).read_text(encoding="utf-8")
            with self.subTest(document=document_name):
                self.assertIn("v0.4.2rc1", text)
                self.assertNotIn("v0.2.0", text)
        for readme_name in ("README.md", "README.zh-CN.md"):
            text = (ROOT / readme_name).read_text(encoding="utf-8")
            self.assertIn("assets/release-v0.4.2rc1.svg", text)
