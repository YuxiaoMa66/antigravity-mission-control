...
class ReleaseIntegrityTests(unittest.TestCase):
    def test_versions_are_synchronized(self):
        ...
        self.assertEqual(common.VERSION, "0.4.2rc3")
        self.assertEqual(__version__, common.VERSION)
        self.assertIn('version = "0.4.2rc3"', pyproject)
        self.assertEqual(package["version"], "0.4.2-rc.3")
        self.assertIn("const VERSION = '0.4.2-rc.3'", npm_cli)
        self.assertIn("const PYTHON_VERSION = '0.4.2rc3'", npm_cli)
    ...
    def test_skill_sources_are_host_neutral(self):
        # Only host-notes.md may name a specific host.
        for path in [ROOT / "SKILL.md", *sorted((ROOT / "references").glob("*.md"))]:
            if path.name == "host-notes.md":
                continue
            with self.subTest(path=path.name):
                self.assertNotIn("Codex", path.read_text(encoding="utf-8").replace("Codex or Claude Code", ""))
    ...
    def test_bilingual_document_pairs_exist(self):
        pairs = (
            ...
            ("docs/releases/v0.4.2rc1.md", "docs/releases/v0.4.2rc1.zh-CN.md"),
            ("docs/releases/v0.4.2rc2.md", "docs/releases/v0.4.2rc2.zh-CN.md"),
            ("docs/releases/v0.4.2rc3.md", "docs/releases/v0.4.2rc3.zh-CN.md"),
        )
    ...
    def test_local_markdown_links_resolve(self):
        markdown_files = [path for path in ROOT.rglob("*.md") if not {".git", "build", "node_modules"} & set(path.parts)]
    ...
    def test_current_release_assets_and_install_refs(self):
        self.assertTrue((ROOT / "assets" / "release-v0.4.2rc3.svg").is_file())
        self.assertTrue((ROOT / "docs" / "visual-design-v0.4.2rc3.md").is_file())
        for document_name in (
            ...
            "docs/releases/v0.4.2rc3.md",
            "docs/releases/v0.4.2rc3.zh-CN.md",
        ):
            text = (ROOT / document_name).read_text(encoding="utf-8")
            with self.subTest(document=document_name):
                self.assertIn("v0.4.2rc3", text)
                self.assertNotIn("v0.2.0", text)
        for readme_name in ("README.md", "README.zh-CN.md"):
            text = (ROOT / readme_name).read_text(encoding="utf-8")
            self.assertIn("assets/release-v0.4.2rc3.svg", text)
