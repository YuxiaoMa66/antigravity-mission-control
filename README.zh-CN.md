...
<p align="center"><img src="assets/release-v0.4.2rc1.svg" width="920" alt="Antigravity Mission Control v0.4.2rc1 操作控制板"></p>

> **v0.4.2rc1（预发布，npm 为 `0.4.2-rc.1`）：** 一次内部重构的候选版本：把近 1900 行的 Python CLI 单文件拆分为多个模块。行为、`--help` 输出和 Skill 均不变，在提升为正式版之前稳定版仍是 v0.4.1。可用 `npx antigravity-mission-control@next install` 试用。运行时路由行为保持 v0.3.0 的 Flash 优先设计。
>
...
3. 如果 AGY 已经可用，使用 `npx antigravity-mission-control@latest install` 安装稳定版；需要固定版本时使用 `@0.4.1`，想试用 v0.4.2rc1 预发布版则使用 `@next`。如果没有 AGY，先解释官方 `--install-agy` 方案，并为安装 AGY 单独征得我的同意。
...
以下为 v0.4.2rc1 命令摘录与流程示意图。路径、任务 ID 和额度均为示例，并非实时运行截图。
