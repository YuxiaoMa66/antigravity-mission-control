...
<p align="center"><img src="assets/release-v0.5.0.svg" width="920" alt="Antigravity Mission Control v0.5.0 操作控制板"></p>

> **v0.5.0：** Python CLI 模块拆分的稳定版，任务状态可信：失败的 AGY 运行不会再被报告为成功（非零退出、`ERROR` 结果和 AGY 自身的 print timeout 都判为失败），任务状态更新不会再竞态，`agy-mc continue` 在 AGY 1.2 上恢复可用，每次发布都会用 `scripts/real_agy_check.py` 针对真实 AGY CLI 验收。运行时路由行为保持 v0.3.0 的 Flash 优先设计。
>
...
3. 如果 AGY 已经可用，使用 `npx antigravity-mission-control@latest install` 安装稳定版；需要固定版本时使用 `@0.5.0`。如果没有 AGY，先解释官方 `--install-agy` 方案，并为安装 AGY 单独征得我的同意。
...
以下为 v0.5.0 命令摘录与流程示意图。路径、任务 ID 和额度均为示例，并非实时运行截图。
