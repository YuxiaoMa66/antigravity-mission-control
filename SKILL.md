...
4. Compose a bounded assignment using [role contracts](references/role-prompts.md), then create the prompt-bound manifest using [approvals](references/approvals.md). Keep prompts outside the project and send them through the CLI, which uses stdin. When scoped editing is needed, bind opt-in workspace scopes (`--allowed-path`, `--forbidden-path`, `--required-check`).
...
6. Independently inspect the actual changes and relevant checks against the original objective. Separate infrastructure errors, worker findings, scope/check enforcement failures (exit code 5) and acceptance. `done` or `done_with_warnings` does not mean the task passed acceptance (`acceptance: not_evaluated`).

## Workspace evidence and correction

The CLI records private before/after Git snapshots, changed-path fingerprints and diff hashes outside the project. When signed scopes are configured, it rejects preflight drift, audits changed paths against boundaries, and executes repository-cataloged checks. Inspect the reported evidence path plus real diffs: snapshots and scoped checks are not a sandbox, proof of ownership, or an automatic acceptance test. Missing, oversized or concurrent evidence is a limitation, not a clean result.
