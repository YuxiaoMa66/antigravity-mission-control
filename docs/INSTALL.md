...
`@latest` follows the stable npm release. Pin `@0.4.1` when reproducibility matters; the npm bootstrapper then resolves the matching verified Git tag internally. Pre-releases are published under `@next`; pin `@0.4.2-rc.2` to reproduce the v0.4.2rc2 pre-release.
