...
The wrapper starts in standard permission handling. If the confirmed roster says the worker needs normal unrestricted tool access, obtain the separate explicit permission confirmation first, then run with `--unrestricted --unrestricted-approved`. Never infer this from `--roster-approved` or silently fall back to unrestricted after a denial.
